...
public class ClientDependenteController {
		...
		@GetMapping("/client/adicionarDependente/{idCliente}")
		public ModelAndView formAddDependente(@PathVariable("idCliente") long idCliente) {
			...
			ModelAndView mav = new ModelAndView("client/adicionarDependente");
		...
		@PostMapping("/client/adicionarDependente/{idCliente}")
		public ModelAndView addDependente(@PathVariable("idCliente") long idCliente, Dependente dependente){
			...
			return new ModelAndView("redirect:/client/infoCliente/"+idCliente);
		}
	
	
		
		//Listar informações e dependentes do cliente
		...
		//Deletar dependente
		@GetMapping("/client/removerDependente/{idDep}")
		public ModelAndView deleteDependente(@PathVariable("idDep") long idDep) {
			...
			return new ModelAndView("redirect:/client/infoCliente/"+idOrigem);
		}
		
		//Editar dependente
		@GetMapping("/client/editarDependente/{idDep}")
		public ModelAndView formEditDependente(@PathVariable("idDep") long idDep ) {
			Dependente editEldep = dependenteRepo.findById(idDep)
				.orElseThrow(() -> new IllegalArgumentException("ID inválido" + idDep));
			ModelAndView model = new ModelAndView("client/editarDependente");
			model.addObject(editEldep);
			return model;
		}
		
		//Salvar dependente editado no Bando de dados
		@PostMapping("/client/editarDependente/{idDep}")
		public ModelAndView editDependente(@PathVariable("idDep") long idDep , Dependente dependente) {	
			...
			return new ModelAndView("redirect:/client/infoCliente/" +cliente.getId());
		}
